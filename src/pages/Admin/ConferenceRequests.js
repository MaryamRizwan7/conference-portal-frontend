        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl p-12 text-center shadow-sm border border-gray-100">
            ...
            <p className="text-gray-500 font-medium text-lg">
              No pending conference requests found.
            </p>
          </div>
        ) : (
                  ...
                  {conferences.map((conference) => (
                    <tr key={conference.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4">
                        ...
                        <div className="text-sm text-gray-500">{conference.conference_name}</div>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-600">{conference.city}, {conference.country}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">{conference.submission_deadline ? conference.submission_deadline.slice(0, 10) : "-"}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">{conference.start_date ? conference.start_date.slice(0, 10) : "-"}</td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center space-x-2">
                          <button onClick={() => handleApprove(conference.id)} className="inline-flex items-center px-4 py-2 bg-green-50 text-green-700 text-xs font-bold rounded-lg hover:bg-green-100 transition-all">Approve</button>
                          <button onClick={() => handleReject(conference.id)} className="inline-flex items-center px-4 py-2 bg-red-50 text-red-700 text-xs font-bold rounded-lg hover:bg-red-100 transition-all">Reject</button>
                        </div>
                      </td>
                    </tr>
                  ))}
